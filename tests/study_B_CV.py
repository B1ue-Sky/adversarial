...
"""For crossval, lookat the val loss. use k-folds CV"""
...
def main (args):
    ...
    # Common definitions
    num_folds   = args.folds #currently no k-folds  validation
    ...
    if args.debug:
        pre_train = cfg['combined']['pretrain']
        lambda_latest = cfg['combined']['model']['lambda_reg']
        print "cfg setting for loss",type(pre_train),pre_train,type(lambda_latest),lambda_latest
    # for lambda_reg in [10]:
    #     plot_adversarial_training_loss(lambda_reg, num_folds, 10,H_prior)
    #     pass
    # plot_adversarial_training_loss(lambda_latest, num_folds, pre_train, H_prior)
    plot_adversarial_training_loss(10., num_folds, 10, H_prior)
...
@profile
@showsave
def plot_classifier_training_loss (num_folds=None, basedir='models/adversarial/classifier/crossval/'):
    """
    ...
    path='output/figures/loss_classifier_CV.pdf'
    return c, args, path
...
def plot_adversarial_training_loss (lambda_reg=10., num_folds=None, pretrain_epochs=10, H_prior=None, basedir='models/adversarial/combined/full/'):
    ...
    path='output/figures/loss_adversarial_CV_lambda{}_{}.pdf'.format(lambda_str, 'full' if num_folds is None else 'cv')
    return c, args, path
