...
def robustness_full (data, args, features, masscut=False, num_bootstrap=5):
    ...
    # here we just use indivadual plot!
...
def compute (data, args, features, var, bins, masscut, num_bootstrap):
    ...
    for bin in zip(bins[:-1], bins[1:]):
        ...
        meanx.append(data.loc[msk_bin & msk_bkg, var].mean())
...
def plot_individual (*argv):
    ...
    # Loop combinations
    for var, metric in itertools.product([PT, None], ['rej', 'jsd']):
        ...
        with TemporaryStyle() as style:
            ...
            if var is not None:
                ...
                for is_simple in [True, False]:#is_sample means signal_low(value low means it is signal)
                    for ifeat, feat in filter(lambda t: is_simple == signal_low(t[1]), enumerate(features)):
                ...
                c.text( ["data p3652,  #it{Hbb} tagging"] + \
                       (['m #in  [80, 140] GeV'] if masscut else []) + \
                       (['Dijets'] if metric == 'jsd' else []),
...
def plot (*argv):
    ...
    with TemporaryStyle() as style:
        ...
        c.pads()[1].text(["Dijets"], ATLAS=False)
